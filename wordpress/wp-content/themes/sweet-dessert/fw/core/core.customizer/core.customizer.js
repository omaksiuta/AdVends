// Sweet Dessert Customizer script
...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.sweet_dessert_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.sweet_dessert_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && sweet_dessert_get_cookie('to_demo_fixed')==1) sweet_dessert_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.sweet_dessert_options_field_info .to_demo_pin', function(e) {
		sweet_dessert_options_fix_demo(jQuery(this).parents('.sweet_dessert_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	sweet_dessert_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.sweet_dessert_options_input,.sweet_dessert_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.sweet_dessert_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		sweet_dessert_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.sweet_dessert_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.sweet_dessert_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .sweet_dessert_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.sweet_dessert_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? SWEET_DESSERT_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.sweet_dessert_options_tab').find('> .sweet_dessert_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="sweet_dessert_options_schemes_order" class="sweet_dessert_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function sweet_dessert_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.sweet_dessert_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.sweet_dessert_options').find('>.sweet_dessert_options_field_info');
...
function sweet_dessert_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.sweet_dessert_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	sweet_dessert_message_confirm(SWEET_DESSERT_STORAGE['to_strings']['scheme_delete_confirm'], SWEET_DESSERT_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'sweet_dessert_options_scheme_delete',
			nonce: SWEET_DESSERT_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.sweet_dessert_options_tab').find('> .sweet_dessert_options_schemes_order').val()
		};
		setTimeout(function(){
			sweet_dessert_message_info(SWEET_DESSERT_STORAGE['to_strings']['recompile_styles'], SWEET_DESSERT_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(SWEET_DESSERT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				sweet_dessert_message_success(SWEET_DESSERT_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+SWEET_DESSERT_STORAGE['to_strings']['reload_page'], SWEET_DESSERT_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				sweet_dessert_message_warning(SWEET_DESSERT_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, SWEET_DESSERT_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function sweet_dessert_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.sweet_dessert_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	sweet_dessert_message_confirm(SWEET_DESSERT_STORAGE['to_strings']['scheme_copy_confirm'], SWEET_DESSERT_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'sweet_dessert_options_scheme_copy',
			nonce: SWEET_DESSERT_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.sweet_dessert_options_tab').find('> .sweet_dessert_options_schemes_order').val()
		};
		...
		jQuery.post(SWEET_DESSERT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				sweet_dessert_message_success(SWEET_DESSERT_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+SWEET_DESSERT_STORAGE['to_strings']['reload_page'], SWEET_DESSERT_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				sweet_dessert_message_warning(SWEET_DESSERT_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, SWEET_DESSERT_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function sweet_dessert_options_fix_demo(info) {
    "use strict";
	var info2 = info.parents('.sweet_dessert_options').toggleClass('to_demo_fixed').find('>.sweet_dessert_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.sweet_dessert_options').append(clonedObj);
		info2 = info.parents('.sweet_dessert_options').find('>.sweet_dessert_options_field_info');
	}
	sweet_dessert_set_cookie('to_demo_fixed', jQuery('.sweet_dessert_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function sweet_dessert_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.sweet_dessert_options_field_info').data('scheme');
