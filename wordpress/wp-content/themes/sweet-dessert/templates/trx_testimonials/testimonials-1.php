...
if ( !function_exists( 'sweet_dessert_template_testimonials_1_theme_setup' ) ) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_template_testimonials_1_theme_setup', 1 );
	function sweet_dessert_template_testimonials_1_theme_setup() {
		sweet_dessert_add_template(array(
			'layout' => 'testimonials-1',
			'template' => 'testimonials-1',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 1/', 'sweet-dessert'),
			'thumb_title'  => esc_html__('Avatar (small)', 'sweet-dessert'),
			'w'		 => 240,
			'h'		 => 240
		));
	}
...
// Template output
if ( !function_exists( 'sweet_dessert_template_testimonials_1_output' ) ) {
	function sweet_dessert_template_testimonials_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php sweet_dessert_show_layout($post_data['post_content']); ?></div>
				<?php if ($post_options['photo']) { ?>
				<div class="sc_testimonial_avatar"><?php sweet_dessert_show_layout($post_options['photo']); ?></div>
				<?php } ?>
		...
		<?php
		if (sweet_dessert_param_is_on($post_options['slider']) || $columns > 1) {
			?></div><?php
		}
	}
}
