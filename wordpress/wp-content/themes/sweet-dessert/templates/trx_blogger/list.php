...
if ( !function_exists( 'sweet_dessert_template_list_theme_setup' ) ) {
	add_action( 'sweet_dessert_action_before_init_theme', 'sweet_dessert_template_list_theme_setup', 1 );
	function sweet_dessert_template_list_theme_setup() {
		sweet_dessert_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'sweet-dessert')
			));
	}
}

// Template output
if ( !function_exists( 'sweet_dessert_template_list_output' ) ) {
	function sweet_dessert_template_list_output($post_options, $post_data) {
		...
		$title = '<li class="sc_blogger_item sc_list_item'.($columns > 1 ? ' column-1_'.esc_attr($columns) : '').'">'
			...
			. '</li>';
		sweet_dessert_show_layout($title);
	}
}
?>