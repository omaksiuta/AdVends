...
function sweet_dessert_theme_customizer(option, val) {
    "use strict";
	...
	if (option == 'body_style') {
		...
		var params = SWEET_DESSERT_STORAGE['co_add_params']!=undefined ? SWEET_DESSERT_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#sweet_dessert-customizer-styles-'+option).length > 0 ? jQuery('#sweet_dessert-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="sweet_dessert-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
