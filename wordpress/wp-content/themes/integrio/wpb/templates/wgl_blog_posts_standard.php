...
$defaults = array(
    ...
    'hide_share' => '',
    'read_more_hide' => '',
    'read_more_text' => esc_html__('Read More','integrio'),
...
list($query_args) = Integrio_Loop_Settings::buildQuery($atts);
...
switch (true) {      
    case (bool)$custom_fonts_blog_content:  $custom_style = true; break;
    case (bool)$custom_fonts_blog_headings: $custom_style = true; break;
    case (bool)$use_custom_heading_color:   $custom_style = true; break;
    case (bool)$use_custom_content_color:   $custom_style = true; break;
    case (bool)$use_custom_main_color:      $custom_style = true; break;   
    case (bool)$use_custom_read_color:      $custom_style = true; break;    
    case (bool)$custom_blog_mask:           $custom_style = true; break;      
    case (bool)$custom_blog_bg_item:        $custom_style = true; break;    
    case (bool)$custom_blog_hover_mask:     $custom_style = true; break;           
    case (bool)$custom_fonts_blog_size_headings: $custom_style = true; break;      
    case (bool)$custom_fonts_blog_size_content:  $custom_style = true; break;    
}
...
if ( (bool)$custom_fonts_blog_content || (bool)$custom_fonts_blog_headings ) {
    ...
    extract( Integrio_GoogleFontsRender::getAttributes( $atts, $this, array('google_fonts_blog', 'google_fonts_blog_headings')));
...
if ( (bool)$custom_style ) {
    ...
    $heading_line_height = !empty($heading_line_height) ? ' line-height:'.$heading_line_height.'px;' : '';
    $heading_font_weight = !empty($heading_font_weight) ? ' font-weight:'.$heading_font_weight.';' : '';
...
$query = Integrio_Theme_Cache::cache_query($query_args);
// Render Items blog
$wgl_def_atts = array(
    'query' => $query,
    ...
    <section class="wgl_cpt_section">
        <div <?php if ((bool)$blog_id) echo 'id="'.esc_attr($blog_id).'"' ?> class="blog-posts <?php echo esc_attr($css_class); ?>">
            ...
            if(!empty($blog_title) || !empty($blog_subtitle)){
                ...
                if(!empty($blog_title)) echo '<h3 class="integrio_module_title blog_title">'.wp_kses( $blog_title, $allowed_html ).'</h3>';
            ...
            echo '<div class="container-grid row '. esc_attr($row_class) .'">';
                echo Integrio_Theme_Helper::render_html($blog_items);
            echo '</div>';
            ?>
        </div>
<?php
...
if ( $blog_navigation == 'pagination' ) {
    echo Integrio_Theme_Helper::pagination('10', $query, $blog_navigation_align);
}
...
if ( $blog_navigation == 'load_more' ) {
    ...
    echo Integrio_Theme_Helper::load_more($wgl_blog_atts, $name_load_more, $class);
}
    echo '</section>';
endif;
