<?php
	...
	// render html
	$output .= '<div class="integrio_module_demo_item'.esc_attr($di_wrap_classes).'">';
		$output .= '<div class="di_title-wrap">';
			$output .= !empty($di_subtitle) ? '<h5 class="di_subtitle">'.esc_html($di_subtitle).'</h5>' : '';
			$output .= !empty($di_title) ? '<a '.$button_attr.'><h5 class="di_title button-read-more">'.esc_html($di_title).'</h5></a>' : '';
		$output .= '</div>';
		if (!empty($di_image)) {
			$output .= '<div class="di_image-wrap">';
				...
				$output .= (bool)$add_button ? '<div class="di_button integrio_module_button wgl_button wgl_button-l btn-gradient"><a class="wgl_button_link" '.$button_attr.'>'.esc_html($di_button_title).'</a></div>' : '';
				$output .= (bool)$coming_soon ? '<h5 class="di_label">'.esc_html__( 'Coming Soon', 'integrio' ).'</h5>' : '';
			$output .= '</div>';
		}
	$output .= '</div>';
