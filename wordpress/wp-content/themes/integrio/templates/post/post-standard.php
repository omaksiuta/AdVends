...
if ( !(bool)$wgl_blog_atts ) {
    $opt_likes = Integrio_Theme_Helper::get_option('blog_list_likes');
    $opt_share = Integrio_Theme_Helper::get_option('blog_list_share');
    $opt_meta = Integrio_Theme_Helper::get_option('blog_list_meta');
    $opt_meta_author = Integrio_Theme_Helper::get_option('blog_list_meta_author');
    $opt_meta_comments = Integrio_Theme_Helper::get_option('blog_list_meta_comments');
    $opt_meta_categories = Integrio_Theme_Helper::get_option('blog_list_meta_categories');
    $opt_meta_date = Integrio_Theme_Helper::get_option('blog_list_meta_date');
    $opt_read_more = Integrio_Theme_Helper::get_option('blog_list_read_more');
    $opt_hide_media = Integrio_Theme_Helper::get_option('blog_list_hide_media');
    $opt_hide_title = Integrio_Theme_Helper::get_option('blog_list_hide_title');
    $opt_hide_content = Integrio_Theme_Helper::get_option('blog_list_hide_content');
    $opt_letter_count = Integrio_Theme_Helper::get_option('blog_list_letter_count');
    $opt_blog_columns = Integrio_Theme_Helper::get_option('blog_list_columns');
...
while ($query->have_posts()) : $query->the_post();          
    ...
    <div class="blog-post <?php echo esc_attr($blog_item_classes); ?>"<?php echo Integrio_Theme_Helper::render_html($blog_attr);?>>
        <div class="blog-post_wrapper">
            ...
            <?php           
                ...
                wp_link_pages(array('before' => '<div class="page-link">' . esc_html__('Pages', 'integrio') . ': ', 'after' => '</div>'));
                ?>
            </div>
        </div>
    </div>
