...
if (!class_exists('Integrio_Single_Post')) {

	class Integrio_SinglePost{
		/**
	     * @var SinglePost
	     */
	    ...
	    public function videobox(){
	    	if($this->post_format === 'video'){
	    		...
	    		if($video_style != 'bg_video'){
					...
					$uniqrel = uniqid();
		    		echo '<div class="integrio_module_videobox button_align-center">';
	    ...
	    public function render_bg ( $link_feature = false, $image_size = 'full',$aq_image = false, $data_animation = null ) {
			...
			$featured_image = Integrio_Theme_Helper::options_compare('featured_image_type', 'mb_featured_image_conditional', 'custom');	
			if ($featured_image == 'replace') {
				$featured_image_replace = Integrio_Theme_Helper::options_compare('featured_image_replace', 'mb_featured_image_conditional', 'custom');	
			}
	    	...
	    	if($this->post_format === 'gallery' && !is_single()){
	    		...
	    		if($this->post_format === 'video' && $video_style == 'bg_video'){
	    			echo Integrio_Theme_Helper::render_html($media);
	    	...
	    	if(is_single() && $video_style != 'bg_video'){
	    		...
	    		if($video_style == 'bg_video'){
			    	...
			    	if(has_post_thumbnail()) {
			    		...
			    		$output .= '<div class="integrio_module_videobox button_align-center animation_circles with_image always-run-animation">';
	    ...
	    public function featured_gallery ($link_feature, $image_size) {
	    	...
	    	$arr = $this->image_size_render($image_size);  
			extract($arr);
			$class = 'blog-post_media-slider_slick integrio_carousel_slick fade_slick';
			$class_module = ' integrio_module_carousel';
			ob_start();
			?>
			...
			<?php
           	...
           	?>
           		<div class="blog-post_quote-author">
           			<?php
           			echo Integrio_Theme_Helper::render_html($autor_avatar);
	    ...
	    public function image_size_render($image_size){
	    	$arr = array();
	    	switch ($image_size) {
	    		case 'integrio-740-560':
	    		$arr['width'] = '740';
	    		$arr['height'] = '560';
	    		break;	 	    		
	    		case 'integrio-700-550':
	    		$arr['width'] = '700';
	    		$arr['height'] = '550';
	    		break;	    		
	    		case 'integrio-440-440':
	    		$arr['width'] = '440';
	    		$arr['height'] = '440';
	    		break;	    		
	    		case 'integrio-420-300':
	    		$arr['width'] = '420';
	    		$arr['height'] = '300';
	    		break;		    		
	    		case 'integrio-180-180':
	    		$arr['width'] = '180';
	    		$arr['height'] = '180';
	    		break;	
	    		default:
	    	...
	    	if($this->show_author_meta || $this->show_comments_meta || $this->show_date_meta || $this->show_category_meta){
		    	...
		    	<div class="meta-wrapper"> 		
					...
					<?php if($this->show_comments_meta) :
						...
						$comments_text = '' . $comments_num == 1 ? esc_html__('Comment', 'integrio') : esc_html__('Comments', 'integrio')  . '';
					?>
					...
					<?php if($this->show_author_meta) : ?>
						...
						<?php echo esc_html__('by', 'integrio'); ?>
	    ...
	    public function render_excerpt ($symbol_count = false, $shortcode = false, $read_more = false, $read_more_text = false) {
			...
			if ((bool)Integrio_Theme_Helper::get_option('blog_post_listing_content') || $shortcode) {
				$post_content = preg_replace( '~\[[^\]]+\]~', '', $post_content);
				$post_content_stripe_tags = strip_tags($post_content);
				$output = Integrio_Theme_Helper::modifier_character($post_content_stripe_tags, $symbol_count, "...");
			} else {
				$output = $post_content;
			}
	    ...
	    public function get_post_views($postID){
		    ...
		    if($count==''){
		    	...
		    	echo '<span class="counts_text">'. esc_html__(' View', 'integrio'). '</span>';
		    }else{
				echo '<span class="counts">'. esc_html($count). '</span>';
		    	echo '<span class="counts_text">'. esc_html__(' Views', 'integrio'). '</span>';
		    }
			echo '</div>';
	    ...
	    public function render_author_info () {
			...
			$name_html = !empty($user_first) || !empty($user_last) ? '<h5 class="author-info_name">'.'<span class="author-excerpt_name">'.esc_html__('About', 'integrio').'</span>'.$user_first.' '.$user_last.'</h5>' : '';
