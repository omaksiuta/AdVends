...
/**
* Integrio Theme Cache
*
*
* @class        Integrio_Theme_Cache
* @version      1.0
* @category Class
* @author       WebGeniusLab
*/

if (!class_exists('Integrio_Theme_Cache')) {
    class Integrio_Theme_Cache{
        ...
        public static function cache_query($args = array()){
            ...
            $custom_query = wp_cache_get( $k, 'integrio_theme' );
            if ( false ===  ($custom_query) ) {
                $custom_query = new WP_Query( $args );
                if ( ! is_wp_error( $custom_query ) && $custom_query->have_posts() ) {
                    wp_cache_set( $k, $custom_query, 'integrio_theme' );
                }
            }
            return $custom_query;       
        }
