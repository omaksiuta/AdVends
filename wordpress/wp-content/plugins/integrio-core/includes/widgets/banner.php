...
private $isMultilingual = FALSE; //Is this site multilingual?
    ...
    function __construct() 
    {
        parent::__construct(
            ...
            esc_html__( 'WGL Banner', 'integrio-core' ), // Name
            array( 'description' => esc_html__( 'WGL Widget', 'integrio-core' ), ) // Args
        );
    ...
    public function enqueue_backend_scripts()
    {
        ...
        wp_enqueue_script('integrio-upload-img', get_template_directory_uri() . '/core/admin/js/img_upload.js', array('jquery'), false, true);
    }
    ...
    public function widget( $args, $instance ) 
    {
        ...
        if ('' === get_post_meta($attachment_id, '_wp_attachment_image_alt', true)) {
            $alt = the_title_attribute(array('before' => esc_html__('Featured image: ', 'integrio-core'), 'echo' => false));
        } else {
        ...
        $widgetClasses = 'integrio_banner-widget';
        $widgetClasses.= ' widget integrio_widget';
        ...
        if ( !empty($button_link) && empty($widgetImg) && empty($title) ) {
            ...
            if ( !empty($text_sub_title) ) $html .= '<p class="banner-widget_text_sub">' . Integrio_Theme_Helper::render_html($text_sub_title) . '</p>';
            if ( !empty($text) ) {
                $html .= '<h2 class="banner-widget_text">' . Integrio_Theme_Helper::render_html($text);
        ...
        echo Integrio_Theme_Helper::render_html($html);
    }
    ...
    public function form( $instance ) {
        ...
        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>"><?php echo esc_html__('Add Logotype image:', 'integrio-core')?></label><br />
            <img class="integrio_media_image" src="<?php if(!empty($instance[$image_name])){echo esc_url( $instance[$image_name] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat integrio_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name) ); ?>" value="<?php echo esc_attr( $image ); ?>">
            <a href="#" class="button integrio_media_upload"><?php esc_html_e('Upload', 'integrio-core'); ?></a>
        </p>

        <p>
          <label for="<?php echo esc_attr( $this->get_field_id($image_name_2) ); ?>"><?php echo esc_html__('Add background image:', 'integrio-core')?></label><br />
            <img class="integrio_media_image" src="<?php if(!empty($instance[$image_name_2])){echo esc_url( $instance[$image_name_2] );} ?>" style="max-width: 100%" />
            <input type="text" class="widefat integrio_media_url" name="<?php echo esc_attr( $this->get_field_name($image_name_2) ); ?>" id="<?php echo esc_attr( $this->get_field_id($image_name_2) ); ?>" value="<?php echo esc_attr( $image_2 ); ?>">
            <a href="#" class="button integrio_media_upload"><?php esc_html_e('Upload', 'integrio-core'); ?></a>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $padding_top ) ); ?>"><?php echo esc_html__('Padding Top: ', 'integrio-core')?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $padding_bottom ) ); ?>"><?php echo esc_html__('Padding Bottom: ', 'integrio-core')?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $text_sub_title ) ); ?>"><?php echo esc_html__('Sub Title:', 'integrio-core')?></label> 
            <textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $text_sub_title ) ); ?>" name="<?php echo esc_attr(  $this->get_field_name( $text_sub_title ) ); ?>" row="2"><?php echo Integrio_Theme_Helper::render_html($text_sub); ?></textarea>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>"><?php echo esc_html__('Title:', 'integrio-core')?></label> 
            <textarea class="widefat" id="<?php echo esc_attr( $this->get_field_id( $text_name ) ); ?>" name="<?php echo esc_attr(  $this->get_field_name( $text_name ) ); ?>" row="2"><?php echo Integrio_Theme_Helper::render_html($text); ?></textarea>
        </p>    
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_tags')); ?>"><?php esc_html_e( 'Show Title Tags' , 'integrio-core' ); ?></label>
        </p>           

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( $title_name ) ); ?>"><?php echo esc_html__('Button title:', 'integrio-core')?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'button_link' ) ); ?>"><?php esc_html_e( 'Button link:', 'integrio-core' ); ?></label> 
