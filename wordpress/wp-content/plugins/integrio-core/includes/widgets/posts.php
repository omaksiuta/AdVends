...
class WGL_Posts_Widget extends WP_Widget {
    ...
    function __construct() {
        /* Widget settings. */
        $widget_ops = array('classname' => 'widget_wgl_posts', 'description' => esc_html__( 'Display recent posts by categories (or related)' , 'integrio-core' ) );

        /* Create the widget. */
        parent::__construct( 'wgl-posts', esc_html__( 'WGL Posts', 'integrio-core' ), $widget_ops );
    }
    ...
    function widget( $args, $instance ) {
        ...
        echo Integrio_Theme_Helper::render_html($before_widget);

        /* Display the widget title if one was input (before and after defined by themes). */
        if ( $title ){
            echo Integrio_Theme_Helper::render_html($before_title);
            echo esc_attr($instance['title']);
            echo Integrio_Theme_Helper::render_html($after_title);
        }
        ?>
            ...
            <?php while($recent_posts->have_posts()): $recent_posts->the_post(); ?>
                <?php
                    ...
                    if ( $show_content == 'true' ) {
                        ...
                        $text = Integrio_Theme_Helper::modifier_character($without_tags, 50, "...");
                    }else{
                        $text = '';
                    }
                ?>
                    ...
                    <div class="recent-posts-content">
                        ...
                        <div class="recent-posts-content_wrapper">
                            <div class="post_title">
                                <a href='<?php esc_url(the_permalink()); ?>' title='<?php esc_attr_e('Permalink to ','integrio-core'); the_title_attribute(); ?>'><?php echo esc_html(the_title($before = '', $after = '', $echo = false)); ?></a>                    
                            </div><!-- post-title -->
    ...
    <?php else : esc_html_e('No posts were found for display','integrio-core');
    endif; ?>

        <?php
        /* After widget (defined by themes). */
        echo Integrio_Theme_Helper::render_html($after_widget);
    ...
    function form($instance)
    {
        ...
        $defaults = array(
            'title'         => esc_html__( 'Recent Posts' , 'integrio-core' ),
            'num_posts'     => 4,
            'categories'    => 'all',
            'show_related'  => 'off',
            'show_image'    => 'on', 
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e( 'Title:' , 'integrio-core' ) ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('num_posts')); ?>"><?php esc_html_e( 'Number of posts:' , 'integrio-core' ); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_related')); ?>"><?php esc_html_e( 'Show related category posts' , 'integrio-core' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>"><?php esc_html_e( 'Show thumbnail image' , 'integrio-core' ); ?></label>
        </p>

        <p style="margin-top: 20px;">
            <label style="font-weight: bold;"><?php esc_html_e( 'Post meta info' , 'integrio-core' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>"><?php esc_html_e( 'Show date' , 'integrio-core' ); ?></label>
        </p>        
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id('show_content')); ?>"><?php esc_html_e( 'Show content' , 'integrio-core' ); ?></label>
        </p>

        <p>
            <label for="<?php echo esc_attr($this->get_field_id('categories')); ?>"><?php esc_html_e( 'Filter by Category:' , 'integrio-core' ); ?></label> 
            <select id="<?php echo esc_attr($this->get_field_id('categories')); ?>" name="<?php echo esc_attr($this->get_field_name('categories')); ?>" class="widefat categories" style="width:100%;">
                <option value='all' <?php if ( 'all' == $instance['categories'] ) echo 'selected="selected"'; ?>><?php esc_html_e( 'All categories' , 'integrio-core' );?></option>
