...
class Integrio_CorePlugin {
    ...
    static function activation_check() {
        if ( ! self::compatible_version() ) {
            ...
            wp_die( __( '"Integrio-core" Plugin compatible with "Integrio" theme only!', 'integrio-core' ) );
        }
    }
    ...
    function disabled_notice() {
       echo '<strong>' . esc_html__( '"Integrio-core" Plugin compatible with "Integrio" theme only!', 'integrio-core' ) . '</strong>';
    } 
...
new Integrio_CorePlugin();

register_activation_hook( __FILE__, array( 'Integrio_CorePlugin', 'activation_check' ) );
 ...
 */
function deactivate_integrio_core() {
    require_once plugin_dir_path( __FILE__ ) . 'includes/class-wgl-core-deactivator.php';
    Integrio_Core_Deactivator::deactivate();
}

register_activation_hook( __FILE__, 'activate_integrio_core' );
register_deactivation_hook( __FILE__, 'deactivate_integrio_core' );
 ...
 * @since    1.0.0
 */
function run_integrio_core() {

    $plugin = new Integrio_Core();
    $plugin->run();
